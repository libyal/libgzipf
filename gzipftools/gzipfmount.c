...
#endif
{
	const char *description = \
		"Use gzipfmount to mount a GZIP file.";

	gzipftools_option_t options[ ] = {
		{ 'h', NULL, "shows this help" },
		{ 'v', NULL, "verbose output to stderr, while gzipfmount will remain running in the foreground" },
	...
	struct fuse_args gzipfmount_fuse_arguments  = FUSE_ARGS_INIT(1, fuse_argv);
#else
	...
	if( option_extended_options != NULL )
	{
		...
		gzipfmount_fuse_arguments.argc = 0;
		gzipfmount_fuse_arguments.argv = NULL;
#endif
	...
	gzipfmount_dokan_options.SingleThread = TRUE;
#else
...
*/
	...
	fprintf(
	 stderr,
	 "No sub system to mount GZIP file format.\n" );
